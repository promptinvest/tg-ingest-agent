...
| **Saved-to-used review metrics (2026-07-17, Batch 4 MET-001)** | The review reports OUTCOMES, not pile sizes (plan v1.1 §16). `review.collect_note_outcomes` (deterministic, no LLM): saved this period (confirmed lifecycle notes — journal entries excluded), distinct notes actually used (`last_used_at` ≥ since; only real uses bump it), note-event counts (archived/restored/kept/deferred/triaged/review-shown/resurfaced/accepted/reminder-proposed/created), lifecycle counts + review-due, upcoming-7d reviews and expiring temporaries, inbox oldest age, **KPI `capture_to_use_rate`** = distinct notes with `use_count>0` / distinct confirmed (all-time), % archived unused, median capture→first-use from the events log (telemetry-window approximation, labeled), and journal entries per journal (entries table for definition-backed, dated messages for plain journals). `chat_text` now LEADS with the outcome block (📥 saved/used/reminders · 🗂 triage · 🔜 upcoming · 📔 journals) and moves the operational metrics (turns, issues, spend, fallbacks, corrections, first-guess accuracy, memory counts) into a «⚙️ Как я работала / Cara health» tail — §16's user/ops split. The engineering `markdown()` gains a "Notes outcomes (saved-to-used, MET-001)" section with the KPI + secondary metrics. **Note→reminder outcome link:** `_note_reminder_title` stashes the resolved DB id as `note_msg_id`; `do_reminder_create` and the capture card's Save+reminder record `note_reminder_proposed` when the DRAFT is staged, and the reminder-confirm commit records `note_reminder_created` (link survives amends). Never optimize for more saves/nudges/entries — the KPI direction is USE. | Deterministic; ops metrics stay in the health tail; KPI favors use, not volume. |
