...
| **Media capture from own photos (2026-07-27, plan B1+B2)** | The boss photographs a book cover / movie poster / a screenshot with a list, and Cara turns it into catalog entries. Design: a picture-only own-photo turn is vision-CLASSIFIED (`media.classify` — cheap, JSON-strict: media/document/other + one description sentence) and, for media, a SEPARATE verbatim EXTRACT pass (`media.extract`: title + kind movie\|book + visible comment per entry; multi-title lists; ≤20 entries/photo, ≤30/card AND a rendered-length budget — `reply()` hard-cuts at 4000 chars, so the card is trimmed to what actually renders, the surplus is dropped **from the staged set too** (consent covers exactly what he saw), and the card states the storage consequence: «сохраню только то, что показано здесь»; the cap/unread/truncation/caption disclosures persist across correction re-staging via the stash). **Owner decisions recorded 2026-07-27 and enforced:** (1) *no media stored at all* — the photo goes to a tmp dir and is deleted in `try/finally` on every path incl. exceptions; no `images`/`files` rows; the **2026-07-16 own-photo retirement stays fully intact** (scope note: this governs the boss's own photos in this flow — forwarded-post media storage is unchanged, guarded by a regression test); (2) *entries are NOTES* — one confirmed note per entry: summary = the verbatim title (RU stays RU), category **`Movies`/`Books`** (auto-created, English names), purpose `reference`, facts with provenance prefixes — `photo:` comments plus `lookup:`/`model:` author\|director\|year\|genre (`media.entry_facts`; extraction vs enrichment are separate calls with separate provenance, and the card labels each field: «на фото: …» \| «(нашла)» \| «(по памяти)»), chunked+embedded for `ask` — which buys list/detail/delete/recategorize/export for free. **Enrichment (B2, before the card renders):** per entry, creator/year/genre from keyless lookups — OpenLibrary title search for books, Wikipedia opensearch+REST summary (wiki picked by the title's script) for movies+books — through `fetch.fetch_json`, the JSON flavor of the SAME SSRF-pinned/deadlined fetch machinery; a hit must MATCH the normalized title exactly (honest-missing beats the wrong book's year shown as «нашла»); parsing is deterministic and defensive against arbitrary external JSON shapes — year via regex, genre only via a fixed in-code vocabulary, creator via EN 'by <Name>' patterns (RU prose declines names, so RU-wiki creator is deliberately skipped); then ONE budgeted chat call (`media._model_fill`, chat model, metered like every call) fills what lookups left, provenance `model`; a field NEITHER source yields stays ABSENT and the card lists it under «не нашла: …». Every kept lookup/model value is washed by the same `media._clean_line` as photo reads. Lookups run inline on the one thread ⇒ budgeted twice: 5 s per-call timeout (fetch wall-clocks each call at 2×) + a 10-call per-batch cap with fail-fast after 2 consecutive transport failures — a 20-title screenshot enriches its head by lookup and hands the tail to the model. NO general web-search API (owner decision 2026-07-27). Deliberate exception to the budget-stop rule, documented in `media.py`: an `LLMError`/`BudgetExceeded` on the OPTIONAL fallback degrades to honest-missing instead of discarding the already-paid-for batch (budget stays authoritative for classify/extract). A kind-flip correction CLEARS and re-enriches that entry's fields (the looked-up director must not resurface labeled «автор»); (3) confirm-BEFORE-store through the existing single-pending-slot flow: one card per photo/batch (✅/✖️ + reply-to-correct — «№2 — книга, не фильм» / «убери №3» parsed deterministically in `media.parse_correction`, negation-aware, never a router guess over untrusted titles — and STRICT (2026-07-27 review fix): a correction must reference a card entry explicitly, and a message naming another object («удали напоминание №2», «удали заметку 2») routes normally, so an open card cannot swallow unrelated requests; a mis-parse fails safe: card intact, message routed); (4) dedup on (category, `media.normalize_title`) — a re-capture MERGES into the existing note and the ack says so; `media.merge_facts` REFRESHES: a new enrichment fact replaces the old fact for the same field (no contradictory year pair survives), photo comments and free facts append. **Md export (B2):** `export what="category"` + `category` («дай md по Movies», router few-shot + manifest note) renders an md catalog table via `media.catalog_markdown` — title, creator, year, genre, comments, added date, missing fields dashed, pipes escaped — for ANY category (a generic note's facts ride the comments column; `parse_catalog_facts` reads the provenance schema back), sent through the existing `tg_send_document` path. Staging: entries live in `kv:media_capture:{chat}`, NOT in the pending payload (rendered into the router prompt — the note-edit rule); the payload carries only `{"n": count}`; buttons work off the stash even when another confirmation holds the slot (the footer then offers the buttons ONLY — a text reply would resolve against the other pending; same after the pending row's 1h TTL, the note-edit precedent); a new card replaces the previous stash and retires its keyboard; no TTL — the card shows exactly what a confirm stores, so an old confirm is still consent to what's displayed. Albums ride the existing buffer; ≤`MAX_LLM_IMAGES` (4) photos classified per batch, said on the card when exceeded. Failure honesty: transport failure ⇒ `llm_error`, an unreadable-but-media photo ⇒ «названия разобрать не смогла» (never an invented title, a missing field stays missing); a partially unread album is disclosed on the card («не смогла прочитать {n} фото» — 2026-07-27 review fix); classify unusable ⇒ the legacy conversational path (which reuses the classify description instead of paying vision twice). Known trade-off (documented): a caption on a *media* photo is not routed as a command while the card is offered — but it is not silently dropped either (2026-07-27 review fix): the card shows the caption and says it was not acted on. | Card per batch; stored ONLY on explicit ✅/«да»; corrections by reply. |
